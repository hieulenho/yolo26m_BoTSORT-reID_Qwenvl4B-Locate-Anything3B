...
param(
    ...
    [string]$TrackingConfig = "configs/legacy/football/track_video_yolo26m_botsort.yaml",
...
if (-not $TrackingConfigWasProvided) {
    switch ($TrackingProfile) {
        "football_high_recall" {
            $TrackingConfig = "configs/legacy/football/track_video_yolo26m_botsort_high_recall.yaml"
        }
        "general_person" {
            $TrackingConfig = "configs/legacy/football/track_video_yolo26m_pretrained_person_botsort_high_recall.yaml"
        }
        default {
            $TrackingConfig = "configs/legacy/football/track_video_yolo26m_botsort.yaml"
        }
    }
}
...
function Invoke-PipelineVideoRender {
    ...
    $PredictionArgs = @(
        "scripts\legacy\build_pipeline_render_predictions.py",
        "--pipeline", $Pipeline,
...
if ($CompleteRenderLabels) {
    ...
    $CompletionArgs = @(
        "scripts\legacy\build_track_label_completion.py",
        "--sequence-name", $SequenceName,
        "--source-video", "$SourcePath",
        "--tracks", "$TracksResolved",
...
if ($NormalizedPipelines -contains "A") {
    ...
    .\scripts\legacy\analyze_tracking_vlm.ps1 `
