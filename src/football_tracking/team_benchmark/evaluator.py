...
def _write_outputs(
    *,
    ...
    _write_confusion_csv(aggregate["team_confusion_matrix"], paths["confusion_csv"])
...
def _write_aggregate_csv(aggregate: dict[str, Any], path: Path) -> None:
    excluded = {"per_team", "team_confusion_matrix"}
    with path.open("w", newline="", encoding="utf-8") as handle:
...
def _write_confusion_csv(matrix: dict[str, dict[str, int]], path: Path) -> None:
    pred_labels = sorted({label for row in matrix.values() for label in row})
    fields = ["gt_team_label", *pred_labels]
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        for gt_label, counts in matrix.items():
            row = {"gt_team_label": gt_label}
